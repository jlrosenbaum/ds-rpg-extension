import { DSRPGUserSettings } from "./app/settings.js";

import { preloadDSRPGHandlebarsTemplates } from "./app/dsrpg-templates.js";
import { dsrpgListeners } from "./app/listeners.js";
import { dsrpgContextMenu } from "./app/context-menu.js";
import { dsrpgSearchFilter } from "./app/search-filter.js";
...
import { dsrpgSpellLevelButtons } from "./app/dsrpg-spell-level-buttons.js";
import { dsrpgHBEnableUpcastFreeSpell } from "./app/dsrpg-hb-upcast-free-spell.js";
...
export class DSRPGSheet extends dnd5e.applications.actor.ActorSheedsrpgCharacter {
	get template() {
		...
		return "modules/ds-rpg-extension/templates/actors/dsrpg-sheet.html";
	}
	...
	static get defaultOptions() {
		...
		return mergeObject(super.defaultOptions, {
			classes: ["dsrpg", "sheet", "actor", "character"],
			blockFavTab: true,
		...
		html.find(".item-control.item-attunement").click(async (event) => {
			...
			if (item.system.attunement == 2) {
				...
				if (count >= actor.system.attributes.attunement.max) {
					ui.notifications.warn(`${game.i18n.format("DSRPG.AttunementWarning", { number: count })}`);
				} else {
...
async function checkDeathSaveStatus(app, html, data) {
	if (data.editable) {
		...
		debug(
			`ds-rpg-extension | checkDeathSaveStatus | current HP Character : ${currentHealth}, success: ${deathSaveSuccess}, failure: ${deathSaveFailure}`
		);
		if (currentHealth <= 0) {
			html.find(".ds-rpg-extension .profile").addClass("dead");
		}
...
async function editProtection(app, html, data) {
	...
	if (game.user.isGM && game.settings.get(CONSTANTS.MODULE_ID, "editGmAlwaysEnabled")) {
		...
		itemContainer.each(function () {
			...
			debug(`ds-rpg-extension | editProtection | hidden: ${hiddenSections}'/ total: ${totalSections}`);
			if (hiddenSections >= totalSections) {
				if (
					...
					$(this).prepend(`<span class="notice">${game.i18n.localize("DSRPG.GmOnlyEdit")}</span>`);
				} else {
					$(this).append(`<span class="notice">${game.i18n.localize("DSRPG.EmptySection")}</span>`);
				}
			}
		});
	} else if (
		...
		itemContainer.prepend(`<span class="notice">${game.i18n.localize("DSRPG.GmOnlyEdit")}</span>`);
		html.find(".effects-list .items-footer, .effects-list .effect-controls").remove();
...
async function addClassList(app, html, data) {
	if (data.editable) {
		if (!game.settings.get(CONSTANTS.MODULE_ID, "classListDisabled")) {
			...
			mergeObject(actor, { "flags.ds-rpg-extension.classlist": classListHtml });
...
async function spellAttackMod(app, html, data) {
	...
	debug(
		`ds-rpg-extension | spellAttackMod | Prof: ${prof ?? ""} / Spell Ability: ${spellAbility ?? ""} / ability Mod: ${
			abilityMod ?? ""
...
async function abbreviateCurrency(app, html, data) {
	html.find(".currency .currency-item label").each(function () {
		...
		debug(
			`ds-rpg-extension | abbreviateCurrency | Currency Abbr: ${
	...
	if (changeMaxPreparedList) {
		if (changeMaxPreparedList.value?.length > 0) {
			...
			if (oldValue != value) {
				...
				await actor.update({
					"flags.ds-rpg-extension.maxPreparedSpells": getProperty(actor, changeMaxPreparedList.key)
				});
...
function markActiveEffects(app, html, data) {
	if (game.settings.get(CONSTANTS.MODULE_ID, "activeEffectsMarker")) {
		...
		for (let item of items) {
			debug(`ds-rpg-extension | markActiveEffects | item: ${item}`);
			if (item.effects.length > 0) {
				let id = item._id;
				debug(`ds-rpg-extension | markActiveEffects | itemId: ${id}`);
...
function spellSlotMarker(app, html, data) {
	...
	html.find(".spellSlotMarker .dot").click(async (ev) => {
		...
		debug(`ds-rpg-extension | spellSlotMarker | spellLevel: ${spellLevel}, index: ${index}`);
		if (spellLevel) {
...
async function setSheetClasses(app, html, data) {
	...
	if (!game.settings.get(CONSTANTS.MODULE_ID, "playerNameEnabled")) {
		html.find(".ds-rpg-extension #playerName").remove();
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "journalTabDisabled")) {
		html.find('.ds-rpg-extension .dsrpg-navigation a[data-tab="journal"]').remove();
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "rightClickDisabled")) {
		if (game.settings.get(CONSTANTS.MODULE_ID, "classicControlsEnabled")) {
			html.find(".ds-rpg-extension .grid-layout .items-list").addClass("alt-context");
		} else {
			html.find(".ds-rpg-extension .items-list").addClass("alt-context");
		}
	}
	// if (game.settings.get(CONSTANTS.MODULE_ID, "classicControlsEnabled")) {
	...
	// }
	if (!game.settings.get(CONSTANTS.MODULE_ID, "classicControlsEnabled")) {
		html.find(".ds-rpg-extension .items-header-controls").remove();
	}
	if (
		game.settings.get(CONSTANTS.MODULE_ID, "portraitStyle") == "pc" ||
		game.settings.get(CONSTANTS.MODULE_ID, "portraitStyle") == "all"
	) {
		html.find(".ds-rpg-extension .profile").addClass("roundPortrait");
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "hpOverlayDisabled")) {
		html.find(".ds-rpg-extension .profile").addClass("disable-hp-overlay");
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "hpBarDisabled")) {
		html.find(".ds-rpg-extension .profile").addClass("disable-hp-bar");
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "inspirationDisabled")) {
		html.find(".ds-rpg-extension .profile .inspiration").remove();
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "inspirationAnimationDisabled")) {
		html.find(".ds-rpg-extension .profile .inspiration label i").addClass("disable-animation");
	}
	...
	if (game.settings.get(CONSTANTS.MODULE_ID, "hideIfZero")) {
		html.find(".ds-rpg-extension .profile").addClass("autohide");
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "exhaustionDisabled")) {
		html.find(".ds-rpg-extension .profile .exhaustion-container").remove();
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "exhaustionOnHover")) {
		html.find(".ds-rpg-extension .profile").addClass("exhaustionOnHover");
	}

	if (game.settings.get(CONSTANTS.MODULE_ID, "inspirationOnHover")) {
		html.find(".ds-rpg-extension .profile").addClass("inspirationOnHover");
	}
	...
	if (!game.settings.get(CONSTANTS.MODULE_ID, "traitsTogglePc")) {
		html.find(".ds-rpg-extension .traits").addClass("always-visible");
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "traitLabelsEnabled")) {
		html.find(".ds-rpg-extension .traits").addClass("show-labels");
	}
	if (game.user.isGM) {
		html.find(".ds-rpg-extension").addClass("isGM");
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "hiddenDeathSavesEnabled") && !game.user.isGM) {
		html.find(".ds-rpg-extension .death-saves").addClass("gmOnly");
	}
...
Hooks.on("renderActorSheet", (app, html, data) => {
	...
	if (game.modules.get("dnd5e-custom-skills")?.active) {
		html.find(".ds-rpg-extension .ability-scores.custom-abilities").removeClass("custom-abilities");
	}

	// WHY THIS ??? PATCH FOR https://github.com/sdenec/ds-rpg-extension/issues/714
	if (
		...
		warn(
			`ds-rpg-extension | renderActorSheet | Patch 'system.attributes.init.bonus' for value ${app.actor.system?.attributes?.init?.bonus}`
		);
	}
});
...
Hooks.once("ready", (app, html, data) => {
	if (!game.modules.get("colorsettings")?.active && game.user?.isGM) {
		...
		const errorText = `ds-rpg-extension | Requires the 'colorsettings' module. Please ${word} it.`.replace(
			"<br>",
			"\n"
		);
		warn(errorText);
...
Hooks.on("renderAbilityUseDialog", (app, html, options) => {
	dsrpgSpellLevelButtons(app, html, options);
	dsrpgHBEnableUpcastFreeSpell(app, html, options);
});
