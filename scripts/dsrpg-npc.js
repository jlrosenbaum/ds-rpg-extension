import { preloadDSRPGHandlebarsTemplates } from "./app/dsrpg-npc-templates.js";

import { dsrpgListeners } from "./app/listeners.js";
import { dsrpgContextMenu } from "./app/context-menu.js";
import { dsrpgShowActorArt } from "./app/show-actor-art.js";
import { dsrpgItemCard } from "./app/itemcard.js";
import { dsrpgAmmoSwitch } from "./app/ammo-switch.js";
import { applyLazyMoney } from "./app/dsrpg-lazy-money.js";
import { applyLazyExp, applyLazyHp } from "./app/dsrpg-lazy-exp-and-hp.js";
import { applyLocksNpcSheet } from "./app/lockers.js";
import { applyColorPickerCustomization } from "./app/color-picker.js";
// import { addFavorites } from "./app/dsrpg-favorites.js";
import { updateExhaustion } from "./app/dsrpg-exhaustion.js";
import CONSTANTS from "./app/constants.js";
import { d20Roll, isLessThanOneIsOne, is_real_number } from "./app/helpers.js";
import LongRestDialog from "./app/dsrpg-npc-long-rest-dialog.js";
import ShortRestDialog from "./app/dsrpg-npc-short-rest-dialog.js";
...
export default class DSRPGNPC extends dnd5e.applications.actor.ActorSheet5eNPC {
	 ...
	 */
	get template() {
		if (!game.user.isGM && this.actor.limited) return "modules/ds-rpg-extension/templates/actors/dsrpg-npc-ltd.html";
		return "modules/ds-rpg-extension/templates/actors/dsrpg-npc.html";
	}
	...
	async getData(options) {
		...
		if (!is_real_number(this.actor.flags[CONSTANTS.MODULE_ID]?.exhaustion)) {
			setProperty(this.actor, `flags.ds-rpg-extension.exhaustion`, 0);
		}
		...
		if (this.actor.flags[CONSTANTS.MODULE_ID].exhaustion === 0) {
			context.exhaustionTooltip = exhaustionTooltipPrefix + `, ${game.i18n.localize("DSRPG.Exhaustion0")}`;
		} else if (this.actor.flags[CONSTANTS.MODULE_ID].exhaustion === 1) {
			context.exhaustionTooltip = exhaustionTooltipPrefix + `, ${game.i18n.localize("DSRPG.Exhaustion1")}`;
		} else if (this.actor.flags[CONSTANTS.MODULE_ID].exhaustion === 2) {
			context.exhaustionTooltip = exhaustionTooltipPrefix + `, ${game.i18n.localize("DSRPG.Exhaustion2")}`;
		} else if (this.actor.flags[CONSTANTS.MODULE_ID].exhaustion === 3) {
			context.exhaustionTooltip = exhaustionTooltipPrefix + `, ${game.i18n.localize("DSRPG.Exhaustion3")}`;
		} else if (this.actor.flags[CONSTANTS.MODULE_ID].exhaustion === 4) {
			context.exhaustionTooltip = exhaustionTooltipPrefix + `, ${game.i18n.localize("DSRPG.Exhaustion4")}`;
		} else if (this.actor.flags[CONSTANTS.MODULE_ID].exhaustion === 5) {
			context.exhaustionTooltip = exhaustionTooltipPrefix + `, ${game.i18n.localize("DSRPG.Exhaustion5")}`;
		} else if (this.actor.flags[CONSTANTS.MODULE_ID].exhaustion === 6) {
			context.exhaustionTooltip = exhaustionTooltipPrefix + `, ${game.i18n.localize("DSRPG.Exhaustion6")}`;
		} else {
			context.exhaustionTooltip = exhaustionTooltipPrefix;
		}

		if (!is_real_number(this.actor.flags[CONSTANTS.MODULE_ID]?.death?.success)) {
			setProperty(this.actor, `flags.ds-rpg-extension.death.success`, 0);
		}
		if (!is_real_number(this.actor.flags[CONSTANTS.MODULE_ID]?.death?.failure)) {
			setProperty(this.actor, `flags.ds-rpg-extension.death.failure`, 0);
		}
	...
	activateListeners(html) {
		...
		dsrpgListeners(html, actor, this);
		dsrpgContextMenu(html, this);
		dsrpgShowActorArt(html, actor);
		if (game.settings.get(CONSTANTS.MODULE_ID, "itemCardsForNpcs")) {
			dsrpgItemCard(html, actor);
		}
		dsrpgAmmoSwitch(html, actor);
		...
		html.find(".portrait-hp-formula span.rollable").mousedown(async (event) => {
			switch (event.which) {
				case 3: {
					...
					debug(`dsrpg-npc | activateListeners | formula: ${formula}`);
					let r = new Roll(formula);
					let term = r.terms;
					debug(`dsrpg-npc | activateListeners | term: ${term}`);
					let averageString = "";
		...
		html.find(".exhaust-level li").click(async (event) => {
			...
			await actor.update({ "flags.ds-rpg-extension.exhaustion": value });
			// TODO strange why i did need this ???
			setProperty(actor, "flags.ds-rpg-extension.exhaustion", value);
	...
	async _rest(chat, newDay, longRest, dhd = 0, dhp = 0) {
		...
		if (longRest || newDay) {
			...
			if (this.actor.flags[CONSTANTS.MODULE_ID].exhaustion > 0) {
				...
				debug("dsrpg-npc | _rest | exhaustion = " + exhaustion);
				await this.actor.update({ "flags.ds-rpg-extension.exhaustion": exhaustion - 1 });
	...
	async _rollDeathSave(options = {}) {
		...
		if (roll.total >= (roll.options.targetValue ?? 10)) {
			...
			if (roll.isCritical) {
				details.updates = {
					"flags.ds-rpg-extension.death.success": 0,
					"flags.ds-rpg-extension.death.failure": 0,
			...
			// Increment successes
			else details.updates = { "flags.ds-rpg-extension.death.success": Math.clamped(successes, 0, 3) };
		}

		// Save failure
		else {
			let failures = (death.failure || 0) + (roll.isFumble ? 2 : 1);
			details.updates = { "flags.ds-rpg-extension.death.failure": Math.clamped(failures, 0, 3) };
			if (failures >= 3) {
				// 3 Failures = death
...
async function checkDeathSaveStatus(app, html, data) {
	if (data.editable) {
		...
		debug(
			`dsrpg-npc | checkDeathSaveStatus | current HP NPC : ${currentHealth}, success: ${deathSaveSuccess}, failure: ${deathSaveFailure}`
		);
		if (currentHealth <= 0) {
			html.find(".ds-rpg-extension.dsrpg-npc .profile").addClass("dead");
		}

		if ((currentHealth > 0 && deathSaveSuccess != 0) || (currentHealth > 0 && deathSaveFailure != 0)) {
			await actor.update({ "flags.ds-rpg-extension.death.success": 0 });
			await actor.update({ "flags.ds-rpg-extension.death.failure": 0 });
		}
	}
}
...
async function toggleItemMode(app, html, data) {
	html.find(".item-toggle").click((ev) => {
		...
		debug(`dsrpg-npc | toggleItemMode | item.type: ${item.type}`);
...
async function setSheetClasses(app, html, data) {
	...
	if (actor.getFlag(CONSTANTS.MODULE_ID, "showNpcPersonalityInfo")) {
		html.find(".ds-rpg-extension.dsrpg-npc .left-notes").removeClass("hidden");
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "journalTabNPCDisabled")) {
		html.find('.ds-rpg-extension.dsrpg-npc .dsrpg-navigation a[data-tab="journal"]').remove();
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "rightClickDisabled")) {
		if (game.settings.get(CONSTANTS.MODULE_ID, "classicControlsEnabled")) {
			html.find(".ds-rpg-extension.dsrpg-npc .grid-layout .items-list").addClass("alt-context");
		} else {
			html.find(".ds-rpg-extension.dsrpg-npc .items-list").addClass("alt-context");
		}
	}
	// if (game.settings.get(CONSTANTS.MODULE_ID, "classicControlsEnabled")) {
	//   dsrpgClassicControls(html);
	// }
	if (!game.settings.get(CONSTANTS.MODULE_ID, "classicControlsEnabled")) {
		html.find(".ds-rpg-extension.dsrpg-npc .items-header-controls").remove();
	}
	...
	if (!game.settings.get(CONSTANTS.MODULE_ID, "restingForNpcsEnabled")) {
		html.find(".ds-rpg-extension.dsrpg-npc .rest-container").remove();
	}
	if (
		game.settings.get(CONSTANTS.MODULE_ID, "portraitStyle") == "npc" ||
		game.settings.get(CONSTANTS.MODULE_ID, "portraitStyle") == "all"
	) {
		html.find(".ds-rpg-extension.dsrpg-npc .profile").addClass("roundPortrait");
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "hpOverlayDisabledNpc")) {
		html.find(".ds-rpg-extension.dsrpg-npc .profile").addClass("disable-hp-overlay");
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "hpBarDisabled")) {
		html.find(".ds-rpg-extension.dsrpg-npc .profile").addClass("disable-hp-bar");
	}
	...
	if (game.settings.get(CONSTANTS.MODULE_ID, "traitsAlwaysShownNpc")) {
		html.find(".ds-rpg-extension.dsrpg-npc .traits").addClass("always-visible");
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "traitLabelsEnabled")) {
		html.find(".ds-rpg-extension.dsrpg-npc .traits").addClass("show-labels");
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "skillsAlwaysShownNpc")) {
		html.find(".ds-rpg-extension.dsrpg-npc .skills-list").addClass("always-visible");
	}
	if (
		token &&
		...
		html.find(".ds-rpg-extension.dsrpg-npc").addClass("linked");
	}
	if (
		token &&
		!token.actor.prototypeToken.actorLink &&
		...
		html.find(".ds-rpg-extension.dsrpg-npc").addClass("unlinked");
	}
	if (
		!token &&
		(game.settings.get(CONSTANTS.MODULE_ID, "linkMarkerNpc") == "unlinked" ||
		...
		html.find(".ds-rpg-extension.dsrpg-npc").addClass("original");
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "exhaustionDisabled")) {
		html.find(".ds-rpg-extension.dsrpg-npc .profile .exhaustion-container").remove();
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "exhaustionOnHover")) {
		html.find(".ds-rpg-extension.dsrpg-npc .profile").addClass("exhaustionOnHover");
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "hiddenDeathSavesEnabled") && !game.user.isGM) {
		html.find(".ds-rpg-extension.dsrpg-npc .death-saves").addClass("gmOnly");
	}
...
async function npcFavorites(app, html, data) {
	...
	// Issue https://github.com/sdenec/ds-rpg-extension/issues/539
...
function spellSlotMarker(app, html, data) {
	...
	html.find(".spellSlotMarker .dot").click(async (ev) => {
		...
		debug(`dsrpg-npc | spellSlotMarker | spellLevel: ${spellLevel}, index: ${index}`);
		if (spellLevel) {
...
Actors.registerSheet("dnd5e", DSRPGNPC, {
	types: ["npc"],
	makeDefault: true
});
...
Hooks.once("ready", () => {
	...
	//   window.BetterRolls.hooks.addActorSheet("DSRPGNPC");
	// }
});

Hooks.on("renderDSRPGNPC", (app, html, data) => {
...
Hooks.on("renderActorSheet", (app, html, data) => {
	...
	// Issue https://github.com/sdenec/ds-rpg-extension/issues/662
	if (game.modules.get("dnd5e-custom-skills")?.active) {
		html.find(".ds-rpg-extension.dsrpg-npc .ability-scores.custom-abilities").removeClass("custom-abilities");
	}
});
