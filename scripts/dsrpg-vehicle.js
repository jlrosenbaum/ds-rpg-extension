...
export class DSRPGVehicle extends dnd5e.applications.actor.ActorSheedsrpgVehicle {
	static get defaultOptions() {
		...
		return mergeObject(super.defaultOptions, {
			classes: ["dsrpg", "sheet", "actor", "vehicle"],
	...
	get template() {
		if (!game.user.isGM && this.actor.limited) {
			return "modules/ds-rpg-extension/templates/actors/dsrpg-vehicle-ltd.html";
		}
		return "modules/ds-rpg-extension/templates/actors/dsrpg-vehicle.html";
	}
...
async function setSheetClasses(app, html, data) {
	if (game.settings.get(CONSTANTS.MODULE_ID, "rightClickDisabled")) {
		if (game.settings.get(CONSTANTS.MODULE_ID, "classicControlsEnabled")) {
			html.find(".ds-rpg-extension.dsrpg-vehicle .grid-layout .items-list").addClass("alt-context");
		} else {
			html.find(".ds-rpg-extension.dsrpg-vehicle .items-list").addClass("alt-context");
		}
	}
	// if (game.settings.get(CONSTANTS.MODULE_ID, "classicControlsEnabled")) {
	...
	// }
	if (!game.settings.get(CONSTANTS.MODULE_ID, "classicControlsEnabled")) {
		html.find(".ds-rpg-extension.dsrpg-vehicle .items-header-controls").remove();
	}
	if (!game.settings.get(CONSTANTS.MODULE_ID, "restingForNpcsEnabled")) {
		html.find(".ds-rpg-extension.dsrpg-vehicle .rest-container").remove();
	}
	if (
		game.settings.get(CONSTANTS.MODULE_ID, "portraitStyle") == "npc" ||
		game.settings.get(CONSTANTS.MODULE_ID, "portraitStyle") == "all"
	) {
		html.find(".ds-rpg-extension.dsrpg-vehicle .profile").addClass("roundPortrait");
	}
	...
	if (game.settings.get(CONSTANTS.MODULE_ID, "hpOverlayDisabledVehicle")) {
		html.find(".ds-rpg-extension.dsrpg-vehicle .profile").addClass("disable-hp-overlay");
	}
	if (game.settings.get(CONSTANTS.MODULE_ID, "hpBarDisabled")) {
		html.find(".ds-rpg-extension.dsrpg-vehicle .profile").addClass("disable-hp-bar");
	}
...
// Register DSRPG Vehicle Sheet and make default vehicle sheet
Actors.registerSheet("dnd5e", DSRPGVehicle, {
	types: ["vehicle"],
	makeDefault: true
});

Hooks.on("renderDSRPGVehicle", (app, html, data) => {
...
Hooks.on("renderActorSheet", (app, html, data) => {
	...
	if (game.modules.get("dnd5e-custom-skills")?.active) {
		html.find(".ds-rpg-extension.dsrpg-vehicle .ability-scores.custom-abilities").removeClass("custom-abilities");
	}
});
