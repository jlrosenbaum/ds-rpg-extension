...
function patchCurrency(currency) {
	if (hasProperty(currency, "pp")) {
		...
		if (getProperty(currency, "pp") !== ppValue) {
			...
			info(`dsrpg-lazymoney | patchCurrency | update pp from '${getProperty(currency, "pp")}' to '${ppValue}'`);
		}
	}
	if (hasProperty(currency, "gp")) {
		...
		if (getProperty(currency, "gp") !== gpValue) {
			...
			info(`dsrpg-lazymoney | patchCurrency | update gp from '${getProperty(currency, "gp")}' to '${gpValue}'`);
		}
	}
	if (hasProperty(currency, "ep")) {
		...
		if (getProperty(currency, "ep") !== epValue) {
			...
			info(`dsrpg-lazymoney | patchCurrency | update ep from '${getProperty(currency, "ep")}' to '${epValue}'`);
		}
	}
	if (hasProperty(currency, "sp")) {
		...
		if (getProperty(currency, "sp") !== spValue) {
			...
			info(`dsrpg-lazymoney | patchCurrency | update sp from '${getProperty(currency, "sp")}' to '${spValue}'`);
		}
	}
	if (hasProperty(currency, "cp")) {
		...
		if (getProperty(currency, "cp") !== cpValue) {
			...
			info(`dsrpg-lazymoney | patchCurrency | update cp from '${getProperty(currency, "cp")}' to '${cpValue}'`);
		}
	}
	return currency;
}
...
function chatLog(actor, money) {
	debug(`dsrpg-lazymoney | chatlog | money: ${money}`);
...
function getCpValue() {
	...
	// if (game.settings.get('ds-rpg-extension', "ignoreElectrum")) {
	// 	cpValue.gp.down = "sp";
	// 	cpValue.sp.up = "gp";
	// 	delete cpValue.ep;
	// }
