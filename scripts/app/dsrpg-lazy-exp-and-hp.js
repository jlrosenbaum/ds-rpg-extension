...
function _onChangeExp(ev) {
	...
	if (newAmountExp < 0 || !is_real_number(newAmountExp)) {
		debug(`dsrpg-lazy-exp-and-hp | _onChangeExp | [0] WARN: The xp value ${newAmountExp} is not a valid number`);
		newAmountExp = exp;
	}
	...
	// }
	if (newAmountExp < 0 || !is_real_number(newAmountExp)) {
		debug(`dsrpg-lazy-exp-and-hp | _onChangeExp | [1] WARN: The xp value ${newAmountExp} is not a valid number`);
		newAmountExp = 0;
	}

	debug(`dsrpg-lazy-exp-and-hp | _onChangeExp | Update system.details.xp.value to ${newAmountExp}`);
...
function _onChangeHp(ev) {
	...
	if (newAmountHpValue < 0 || !is_real_number(newAmountHpValue)) {
		debug(
			`dsrpg-lazy-exp-and-hp | _onChangeHp | [2] WARN: The hp.value value ${newAmountHpValue} is not a valid number`
		);
		newAmountHpValue = hp;
	}
	...
	if (game.settings.get(CONSTANTS.MODULE_ID, "lazyHpForceHpValueLimit2")) {
		...
		if (newAmountHpValue < 0 || !is_real_number(newAmountHpValue)) {
			debug(
				`dsrpg-lazy-exp-and-hp | _onChangeHp | [3] WARN: The hp.value value ${newAmountHpValue} is not a valid number`
			);
			newAmountHpValue = 0;
		}
		if (newAmountHpTemp < 0 || !is_real_number(newAmountHpTemp)) {
			debug(
				`dsrpg-lazy-exp-and-hp | _onChangeHp | [4] WARN: The hp.temp value ${newAmountHpTemp} is not a valid number`
			);
			newAmountHpTemp = 0;
		}
		if (newAmountHpTempMax < 0 || !is_real_number(newAmountHpTempMax)) {
			debug(
				`dsrpg-lazy-exp-and-hp | _onChangeHp | [5] WARN: The hp.tempmax value ${newAmountHpTempMax} is not a valid number`
			);
			newAmountHpTempMax = 0;
		}

		debug(`dsrpg-lazy-exp-and-hp | _onChangeHp | Update system.attributes.hp.value to ${newAmountHpValue}`);
		debug(`dsrpg-lazy-exp-and-hp | _onChangeHp | Update system.attributes.hp.temp to ${newAmountHpTemp}`);
		debug(`dsrpg-lazy-exp-and-hp | _onChangeHp | Update system.attributes.hp.tempmax to ${newAmountHpTempMax}`);
		...
		if (newAmountHpValue < 0 || !is_real_number(newAmountHpValue)) {
			debug(
				`dsrpg-lazy-exp-and-hp | _onChangeHp | [6] WARN: The hp.value value ${newAmountHpValue} is not a valid number`
			);
			newAmountHpValue = 0;
		}
...
function _onChangeHpMax(ev) {
	...
	if (newAmountHpMax < 0 || !is_real_number(newAmountHpMax)) {
		debug(
			`dsrpg-lazy-exp-and-hp | _onChangeHpMax | [7] WARN: The hp.max value ${newAmountHpMax} is not a valid number`
		);
	...
	if (newAmountHpMax < 0 || !is_real_number(newAmountHpMax)) {
		debug(
			`dsrpg-lazy-exp-and-hp | _onChangeHpMax | [8] WARN: The hp.value value ${newAmountHpMax} is not a valid number`
		);
		newAmountHpMax = 0;
	}

	debug(`dsrpg-lazy-exp-and-hp | _onChangeHpMax | Update system.attributes.hp.max to ${newAmountHpMax}`);
...
function _onChangeHpForceHpValueLimit1(ev) {
	...
	if (!is_real_number(newAmountHpValue)) {
		debug(
			`dsrpg-lazy-exp-and-hp | _onChangeHpForceHpValueLimit1 | [9] WARN: The hp.value value ${newAmountHpValue} is not a valid number`
		);
		newAmountHpValue = hp;
	}
	...
	if (newAmountHpValue < 0 || !is_real_number(newAmountHpValue)) {
		debug(
			`dsrpg-lazy-exp-and-hp | _onChangeHpForceHpValueLimit1 | [10] WARN: The hp.value value ${newAmountHpValue} is not a valid number`
		);
		newAmountHpValue = 0;
	}
	if (newAmountHpTemp < 0 || !is_real_number(newAmountHpTemp)) {
		debug(
			`dsrpg-lazy-exp-and-hp | _onChangeHpForceHpValueLimit1 | [11] WARN: The hp.temp value ${newAmountHpTemp} is not a valid number`
		);
		newAmountHpTemp = 0;
	}
	if (newAmountHpTempMax < 0 || !is_real_number(newAmountHpTempMax)) {
		debug(
			`dsrpg-lazy-exp-and-hp | _onChangeHpForceHpValueLimit1 | [12] WARN: The hp.tempmax value ${newAmountHpTempMax} is not a valid number`
		);
		newAmountHpTempMax = 0;
	}
	...
	debug(
		`dsrpg-lazy-exp-and-hp | _onChangeHpForceHpValueLimit1 | Update system.attributes.hp.value to ${newAmountHpValue}`
	);
...
function _onChangeHpForceHpValueLimit2(ev) {
	...
	if (newAmountHpValue < 0 || !is_real_number(newAmountHpValue)) {
		debug(
			`dsrpg-lazy-exp-and-hp | _onChangeHpForceHpValueLimit2 | [13] WARN: The hp.value value ${newAmountHpValue} is not a valid number`
		);
		newAmountHpValue = hp;
	}
	...
	if (newAmountHpValue < 0 || !is_real_number(newAmountHpValue)) {
		debug(
			`dsrpg-lazy-exp-and-hp | _onChangeHpForceHpValueLimit2 | [14] WARN: The hp.value value ${newAmountHpValue} is not a valid number`
		);
		newAmountHpValue = 0;
	}
	if (newAmountHpTemp < 0 || !is_real_number(newAmountHpTemp)) {
		debug(
			`dsrpg-lazy-exp-and-hp | _onChangeHpForceHpValueLimit2 | [15] WARN: The hp.temp value ${newAmountHpTemp} is not a valid number`
		);
		newAmountHpTemp = 0;
	}
	if (newAmountHpTempMax < 0 || !is_real_number(newAmountHpTempMax)) {
		debug(
			`dsrpg-lazy-exp-and-hp | _onChangeHpForceHpValueLimit2 | [16] WARN: The hp.tempmax value ${newAmountHpTempMax} is not a valid number`
		);
		newAmountHpTempMax = 0;
	}

	debug(
		`dsrpg-lazy-exp-and-hp | _onChangeHpForceHpValueLimit2 | Update system.attributes.hp.value to ${newAmountHpValue}`
	);
	debug(
		`dsrpg-lazy-exp-and-hp | _onChangeHpForceHpValueLimit2 | Update system.attributes.hp.temp to ${newAmountHpTemp}`
	);
	debug(
		`dsrpg-lazy-exp-and-hp | _onChangeHpForceHpValueLimit2 | Update system.attributes.hp.tempmax to ${newAmountHpTempMax}`
	);
...
function _onChangeHpForceHpTempLimit2(ev) {
	...
	if (newAmountHpTemp < 0 || !is_real_number(newAmountHpTemp)) {
		debug(
			`dsrpg-lazy-exp-and-hp | _onChangeHpForceHpTempLimit2 | [17] WARN: The hp.temp value ${newAmountHpTemp} is not a valid number`
		);
	...
	if (newAmountHpTemp < 0 || !is_real_number(newAmountHpTemp)) {
		debug(
			`dsrpg-lazy-exp-and-hp | _onChangeHpForceHpTempLimit2 | [18] WARN: The hp.temp value ${newAmountHpTemp} is not a valid number`
		);
		newAmountHpTemp = 0;
	}

	debug(
		`dsrpg-lazy-exp-and-hp | _onChangeHpForceHpTempLimit2 | Update system.attributes.hp.temp to ${newAmountHpTemp}`
	);
...
// 				info(`dsrpg-lazy-exp-and-hp | preUpdateActor | [0] update system.attributes.hp.value from '${hpValue}' to '${getProperty(update, "system.attributes.hp.value")}'`);
// 			}
...
// 					info(`dsrpg-lazy-exp-and-hp | preUpdateActor | [1] update system.attributes.hp.value from '${hpValue}' to '${getProperty(update, "system.attributes.hp.value")}'`);
// 				}
// 			}
// 			if(!is_real_number(getProperty(update, "system.attributes.hp.value"))) {
// 				setProperty(update, "system.attributes.hp.value", 0);
// 				info(`dsrpg-lazy-exp-and-hp | preUpdateActor | [2] update system.attributes.hp.value from '${getProperty(update, "system.attributes.hp.value")}' to number 0`);
// 			}
// 		}
// 	}
...
// 				info(`dsrpg-lazy-exp-and-hp | preUpdateActor | [0] update system.attributes.hp.max from '${hpMaxValue}' to '${getProperty(update, "system.attributes.hp.max")}'`);
// 			}
// 			// Module compatibility with https://foundryvtt.com/packages/link-item-resource-5e
...
// 					info(`dsrpg-lazy-exp-and-hp | preUpdateActor | [1] update system.attributes.hp.max from '${hpMaxValue}' to '${getProperty(update, "system.attributes.hp.max")}'`);
// 				}
// 			}
// 			if(!is_real_number(getProperty(update, "system.attributes.hp.max"))) {
// 				setProperty(update, "system.attributes.hp.max", 0);
// 				info(`dsrpg-lazy-exp-and-hp | preUpdateActor | [2] update system.attributes.hp.max from '${getProperty(update, "system.attributes.hp.max")}' to number 0`);
// 			}
// 		}
// 	}
...
// 				info(`dsrpg-lazy-exp-and-hp | preUpdateActor | [0] update system.details.xp.value from '${xpValue}' to '${getProperty(update, "system.details.xp.value")}'`);
// 			}
// 			// Module compatibility with https://foundryvtt.com/packages/link-item-resource-5e
...
// 					info(`dsrpg-lazy-exp-and-hp | preUpdateActor | [1] update system.details.xp.value from '${xpValue}' to '${getProperty(update, "system.details.xp.value")}'`);
// 				}
// 			}
// 			if(!is_real_number(getProperty(update, "system.details.xp.value"))) {
// 				setProperty(update, "system.details.xp.value", 0);
// 				info(`dsrpg-lazy-exp-and-hp | preUpdateActor | [2] update system.details.xp.value from '${getProperty(update, "system.details.xp.value")}' to number 0`);
// 			}
// 		}
// 	}
// });
