...
export const dsrpgSpellLevelButtons = async function (app, html, options) {
	if (
        ...
        `);
			...
			$(optionsApplication._element[0])
				...
				.each(function () {
					...
					if (!availableTextSlotsFounded) {
						warn(
							`dsrpg-spell-level-buttons | dsrpgSpellLevelButtons | Cannot find the spell slots on text '${$(
								this
							).text()}' with ${/\(\d+\s\w+\)/}`
						);
					}
					...
					if (value == "pact") {
						...
						if (!availablePactSlotsFounded) {
							warn(
								`dsrpg-spell-level-buttons | dsrpgSpellLevelButtons | Cannot find the pact slots on text '${$(
									this
								).text()}' with ${/\d/}`
							);
						}
