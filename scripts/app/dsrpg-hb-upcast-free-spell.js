...
export const dsrpgHBEnableUpcastFreeSpell = async function (app, html, options) {
	if (game.settings.get(CONSTANTS.MODULE_ID, "hbEnableUpcastFreeSpell")) {
		if (app?.item?.type != "spell") {
			debug(`dsrpgHBEnableUpcastFreeSpell | Nevermind if this isn't a spell`);
			return; // Nevermind if this isn't a spell
		}
		if (html.find('[name="consumeSpellSlot"]').length == 0) {
			debug(`dsrpgHBEnableUpcastFreeSpell | Nevermind if this is a cantrip`);
			return; // Nevermind if this is a cantrip
		}

		let tooltip = game.i18n.localize("DSRPG.LevelBumpTooltip");

		if(game.settings.get(CONSTANTS.MODULE_ID, "hbSetFeaturesForUpcastFreeSpell") && app.item?.actor){
			debug(`dsrpgHBEnableUpcastFreeSpell | hbSetFeaturesForUpcastFreeSpell check`);
			...
			if(!check) {
				debug(`dsrpgHBEnableUpcastFreeSpell | hbSetFeaturesForUpcastFreeSpell check is failed`);
				return;
			}
		...
		//     <label class="checkbox"><input type="checkbox" name="freeUpcast" />${game.i18n.localize("DSRPG.LevelBump")}</label>
		//   </div>`);
...
Hooks.on("dnd5e.preItemUsageConsumption", (item, config, options) => {
	if (game.settings.get(CONSTANTS.MODULE_ID, "hbEnableUpcastFreeSpell")) {
		if (item?.type != "spell") {
			debug(`dsrpgHBEnableUpcastFreeSpell | dnd5e.preItemUsageConsumption | Nevermind if this isn't a spell`);
			return; // Nevermind if this isn't a spell
		}
