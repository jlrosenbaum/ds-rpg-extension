...
export const dsrpgItemCard = function (html, actor) {
	...
	let itemCardDelayCard = async (event) => {
		debug(`dsrpg-itemcard | itemCardDelayCard | itemCardDelaying card: ${itemCardDelay} ms`);
	...
	async function showCard(event) {
		...
		if (!item) {
			warn(
				`dsrpg-context-menu | showCard | no item found on actor '${actor.name}' with id '${li.data(
					"item-id"
				)}'`
			);
			return;
	...
	$("#item-info-container").on("click", ".button", function (e) {
		...
		$(`.ds-rpg-extension .item[data-item-id='${itemId}'] .item-buttons .button[data-action='${action}']`).trigger(e);
	});
};
