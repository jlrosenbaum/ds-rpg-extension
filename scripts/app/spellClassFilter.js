...
const classesConfiguration = {
	knight: "DSRPG.ClassKnight",
    mercenary: "DSRPG.ClassMercenary",
    assassin: "DSRPG.ClassAssassin",
    warrior: "DSRPG.ClassWarrior",
    thief: "DSRPG.ClassThief",
    herald: "DSRPG.ClassHerald",
    cleric: "DSRPG.ClassCleric",
    sorcerer: "DSRPG.ClassSorcerer",
    pyromancer: "DSRPG.ClassPyromancer",
    deprived: "DSRPG.ClassDeprived"
};
...
export async function applySpellClassFilterItemSheet(app, html, itemData) {
	...
	if (type == "spell") {
		...
		const spellClassForm = await renderTemplate(
			"modules/ds-rpg-extension/templates/items/dsrpg-spell-class-filter-form.html",
			{
	...
	if (type == "character") {
		...
		if (user_setting_filterSelect) {
			const actorClassFilter = await renderTemplate(
				"modules/ds-rpg-extension/templates/actors/parts/dsrpg-spellbook-class-filter.html",
				{
					SCF: classesConfiguration,
					actor,
					flags: flags,
