...
import { isItemFavorite } from "./dsrpg-favorites.js";

export const dsrpgContextMenu = function (html, sheet) {
	...
	Hooks.on("dnd5e.getActiveEffectContextOptions", (effect, contextOptions) => {
		...
		if (actor?.isOwner) {
			...
			if (game.settings.get(CONSTANTS.MODULE_ID, "rightClickDisabled")) {
				...
				let dsrpgContextOptions = _getActiveEffectContextOptions(effect);
				contextOptions = dsrpgContextOptions.concat(contextOptions);
			}
	...
	Hooks.on("dnd5e.getItemContextOptions", (item, contextOptions) => {
		...
		if (actor?.isOwner) {
			...
			if (game.settings.get(CONSTANTS.MODULE_ID, "rightClickDisabled")) {
				if (item.type === "spell" && !item.actor.getFlag(CONSTANTS.MODULE_ID, "ds-rpg-extension.spellbook-grid")) {
					contextOptions = [];
				} else if (item.type !== "spell" && !item.actor.getFlag(CONSTANTS.MODULE_ID, "inventory-grid")) {
					contextOptions = [];
				} else {
					//merge new options with dsrpg options
					let dsrpgContextOptions = _getItemContextOptions(item);
					contextOptions = dsrpgContextOptions.concat(contextOptions);
				}
			} else {
				//merge new options with dsrpg options
...
const _getItemContextOptions = function (item) {
	...
	if ("attunement" in item.system && item.system.attunement !== CONFIG.DND5E.attunementTypes.NONE) {
		...
		// });
		options.push({
			name: isAttuned ? "DSRPG.Deattune" : "DSRPG.Attune",
			icon: isAttuned
	...
	if ("equipped" in item.system) {
		...
		options.push({
			name: isEquipped ? "DSRPG.Unequip" : "DSRPG.Equip",
			icon: isEquipped
	...
	if ("preparation" in item.system) {
		...
		if (allowCantripToBePreparedOnContext) {
			if (item.system.preparation.mode != "always") {
				options.push({
					name: isActive ? "DSRPG.Unprepare" : "DSRPG.Prepare",
	...
	if (isCharacter) {
		...
		options.push({
			name: isFav ? "DSRPG.RemoveFav" : "DSRPG.AddFav",
			icon: isFav
			...
			callback: () => {
				...
				if (!item) {
					warn(`dsrpg-context-menu | _getItemContextOptions | Item no founded!`);
					return;
				}
				let isFav = isItemFavorite(item);
