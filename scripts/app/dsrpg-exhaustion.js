...
export async function updateExhaustion(actorEntity) {
	...
	if (game.settings.get(CONSTANTS.MODULE_ID, "exhaustionEffectsEnabled") == "dsrpg") {
		...
		for (const effectEntity of actorEntity.effects) {
			if (typeof effectEntity.getFlag(CONSTANTS.MODULE_ID, "exhaustion") === "number") {
				...
				debug(`dsrpg-exhaustion | updateExhaustion | currentExhaustion: ${currentExhaustion}`);
		...
		async function createExhaustionEffect() {
			if (exhaustion > 0) {
				debug(`dsrpg-exhaustion | createExhaustionEffect | create Effect exhaustion lv: ${exhaustion}`);
				let effectChange = {
					disabled: false,
					label: effectName,
					icon: icon,
			...
			for (let i = 1; i <= levels; i++) {
				...
				if (tier != effectNameCustom) {
					if (game.dfreds.effectInterface.hasEffectApplied(tier, actorToCheck.uuid)) {
						debug(`dsrpg-exhaustion | createExhaustionEffect | tier: ${tier}`);
						const contextEffect = {
							effectName: tier,
	...
	if (game.settings.get(CONSTANTS.MODULE_ID, "exhaustionEffectsEnabled") == "cub") {
		if (game.modules.get("combat-utility-belt")?.active) {
			...
			if (exhaustion != 0) {
				if (index == -1) {
					ui.notifications.warn(`${game.i18n.localize("DSRPG.Settings.CustomExhaustionEffect.warning")}`);
					return;
				}
...
Hooks.on(`dnd5e.restComplete`, (actorEntity, data) => {
	...
	if (data.longRest) {
		if (actor.type === "character") {
			...
			if (exhaustion > 0) {
				actor.update({ "flags.ds-rpg-extension.exhaustion": exhaustion - 1 });
			}
		} else {
...
Hooks.on(`createActiveEffect`, (effect, data, id) => {
	if (
		...
		if (effectName.includes(game.settings.get(CONSTANTS.MODULE_ID, "exhaustionEffectCustom"))) {
			debug("dsrpg-exhaustion | createActiveEffect | effectName = " + effectName);
			if (actor.type === "character") {
				let exhaustion = effectName.slice(-1);
				if (actor.system.attributes.exhaustion != exhaustion) {
					debug("dsrpg-exhaustion | createActiveEffect | exhaustion = " + exhaustion);
				...
				if (actor.flags[CONSTANTS.MODULE_ID].exhaustion != exhaustion) {
					debug("dsrpg-exhaustion | createActiveEffect | exhaustion = " + exhaustion);
					actor.update({ "flags.ds-rpg-extension.exhaustion": exhaustion });
				}
			} else {
	...
	if (
		...
		if (effectLabel.includes(effectName)) {
			...
			if (actor.type === "character") {
				...
				if (actor.flags.dsrpg - sheet.exhaustion != 0) {
					actor.update({ "flags.ds-rpg-extension.exhaustion": 0 });
				}
			} else {
