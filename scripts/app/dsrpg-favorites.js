 ...
 * adapted for the DSRPGSheet from
...
import { dsrpgAmmoSwitch } from "./ammo-switch.js";
import { applyColorPickerCustomization } from "./color-picker.js";
import CONSTANTS from "./constants.js";
import { dsrpgContextMenu } from "./context-menu.js";
...
export const addFavorites = async function (app, html, data, position) {
		...
		if (app.options.editable) {
			let favBtn = $(
				`<a class="item-control item-fav ${isFav ? "active" : ""}" title="${
					isFav ? game.i18n.localize("DSRPG.RemoveFav") : game.i18n.localize("DSRPG.AddFav")
			...
			favBtn.click((ev) => {
				...
				if (!item) {
					warn(`dsrpg-favorites | addFavorites | Item no founded!`);
					return;
				}
				let isFav = isItemFavorite(item);

				item.update({
					"flags.ds-rpg-extension.favorite": !isFav
				});
	...
	if (renderFavTab) {
		...
		await loadTemplates(["modules/ds-rpg-extension/templates/favorites/dsrpg-favorite-item.html"]);
		let favHtml = $(
			await renderTemplate("modules/ds-rpg-extension/templates/favorites/dsrpg-favorite-template.html", context)
		);

		// Activating favorite-list events
		dsrpgContextMenu(favHtml, app);
		...
		if (app.options.editable) {
				...
				if (item.system.attunement == 2) {
					...
					if (app.actor.system.details.attunedItemsCount >= app.actor.system.details.attunedItemsMax) {
						...
						ui.notifications.warn(
							`${game.i18n.format("DSRPG.AttunementWarning", {
								number: count
							})}`
						);
					} else {
			...
			favHtml.find(".item").on("drop", (ev) => {
				...
				debug(
					`dsrpg-favorites | addFavorite | dragSource: ${dragSource} // siblings: ${siblings} // targetID: ${targetId} // dragTarget: ${dragTarget}`
				);
				if (dragTarget === undefined) {
					// catch trying to drag from one list to the other, which is not supported
					debug(
						`dsrpg-favorites | addFavorite | catch trying to drag from one list to the other, which is not supported, folder not supported`
					);
					return;
				}
				...
				const sortUpdates = SortingHelpers.performIntegerSort(dragSource, {
					target: dragTarget,
					siblings: siblings,
					sortKey: "flags.ds-rpg-extension.sort"
				});
		...
		dsrpgAmmoSwitch(favHtml, app.actor);
