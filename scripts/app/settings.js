...
export class DSRPGUserSettings extends FormApplication {
	static init() {
		settingsList();
	}
	...
	static get defaultOptions() {
		return {
			...
			template: "modules/ds-rpg-extension/templates/settings.html",
			height: 500,
			title: game.i18n.localize("DSRPG.Settings.SheetMenu.title"),
			width: 600,
			classes: ["dsrpg", "settings"],
			tabs: [
				{
					navSelector: ".tabs",
	...
	getSettingsData() {
		// debug(game.settings.get('ds-rpg-extension'))
		const settings = [
			"ammoEquippedOnly",
			"activeEffectsMarker",
			"classListDisabled",
		...
		// return game.settings.get('ds-rpg-extension', 'user-settings');
		let data = {};
	...
	activateListeners(html) {
		...
		html.find("input#exhaustionEffectIcon").on("change", function (e) {
			...
			if (e.target.value == "" || e.target.value == null) {
				e.target.value = "modules/ds-rpg-extension/images/exhaustion.svg";
			}
		});
	}
...
Hooks.on("renderDSRPGUserSettings", () => {
	if (!game.user.isGM) {
		document.querySelectorAll(".dsrpg.settings .gm-only").forEach(function (el) {
			el.remove();
		});
	}
});
